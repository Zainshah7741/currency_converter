...
while (condition) {
  ...
  if (isNaN(exchangedAmount)) {
    ...
    console.log(
      `\n\t ${chalk.yellowBright(userAnswer.fisrtCur)} To ${chalk.hex(
        "#43e802"
      )(userAnswer.secondCur)}`
    );
    console.log(
      `\n\t${chalk.hex("#2196ED")(
        "Converted Amount is"
      )}  : ${chalk.underline.bold.yellowBright(
        userAnswer.secondCur
      )} = ${chalk.underline.bold.hex("#DA00EB")(exchangedAmount.toFixed(2))}\n`
    );
  }
  ...
  const more = await inquirer.prompt({
    ...
    type: "list",
    message:
      chalk.blue.bold.underline("Do You Want Conve") +
  ...
  if (more.more == "Yes") {
    ...
    condition = false;
    console.log(
      `\n \t ${chalk.underline.bold.italic.yellowBright(
